...
"""
WHOWHEN = "Guillem Portella, June 2018"
...
def do_download_sra(*, to_dw):
    ...
    if "sra_dict" in to_dw:
        for name, run_id in to_dw['sra_dict'].items():
            cmd = " ".join([my_prefetch, '-v', run_id, "&& "])
...
def parse_input_file(inp_f: str, verbose: bool) -> Dict[str, Dict[str, str]]:
    ...
    my_config = {'prefetch_exe': my_prefetch,
                 'fqdump_exe': my_fqdump}

    if not config.has_section('SRR_code') or \
            len(config.options('SRR_code')) < 1:
